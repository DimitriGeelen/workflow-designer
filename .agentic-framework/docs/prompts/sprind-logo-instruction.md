Add the SPRIND funder credit to the site footer in `/opt/1409-sprind`.

Streichliste is funded in the SPRIND Challenge „Deutschland, was geht?". The
footer needs a credit saying so. The page is generated by `tools/site_build.py`
— edit the generator, not `site/*.html`.

**The mark.** Save as `site/assets/sprind-wordmark.svg` and inline it in the
footer markup. Inline it rather than using `<img src>`: it is
`fill="currentColor"`, so inlined it follows the container's `color` and one
file serves both light and dark grounds. As an `<img>`, `currentColor` stops
resolving and you would need two fixed-ink copies.
...
```css
.fw-credit{ display:flex; align-items:center; gap:22px; flex-wrap:wrap;
  padding:22px 0; border-top:1px solid var(--rule); }
.fw-mark{ flex:none; width:136px; max-width:180px; color:var(--ink); }
.fw-mark svg{ display:block; width:100%; height:auto; }
.fw-line{ margin:0; max-width:44ch; font-family:var(--font-mono);
  font-size:12px; line-height:19px; letter-spacing:.02em; color:var(--ink-2); }
...
1. **Footer only.** Never beside a finding; never on Die Liste, Die Belege or
   Die Struktur.
2. **Wordmark size** — never wider than 180px.
3. **`ink` on light grounds, `paper` on dark.** Never `blue`, `steel` or any
   evidence token — those state how strong the evidence is, and a funder makes
   no claim about evidence.
4. **It says who funded the work, never that SPRIND vouches for a finding.** On
   a site whose headline number is zero-confirmed, that is the one reading a
   credit must not invite.

**Do not ship the wording yet.** „Gefördert in der SPRIND Challenge »Deutschland,
was geht?«" is factually accurate, but the exact German formula is fixed by
SPRIND's corporate-design manual. Build the slot, put the placeholder in, and
flag it — ship only wording the operator has confirmed with SPRIND.

Do not add SPRIND's BMFTR ministry lockup: that records who funds SPRIND, not
who funds Streichliste.

Provenance: extracted 2026-09-24 from `https://www.sprind.org/en`, where the
mark is an inline `<svg>` (136×19, one compound path). Cleaned only — framework
attributes and utility classes removed, `viewBox` casing fixed, `role`/`<title>`
added. Geometry untouched.
