...
def main() -> int:
    ...
    for err in errors:
        msg += f"    - {err}\n"
    msg += (
        "\n  Add the fields to frontmatter:\n"
        "    target_blast_radius: <int 0..9>\n"
        "    voi_score: <float 0..1>   # NOT 0.5 unless you mean it\n\n"
        "  On BOTH fields (T-624/T-625): together they fix the task's entire BVP\n"
        "  position — target_blast_radius is the cost axis, voi_score the value\n"
        "  axis. Measured 2026-08-29: the SAME 38 of 41 inceptions carried the\n"
        "  template default on both, so the ranking was the template's opinion.\n\n"
        "  On voi_score (T-624): for an inception this field IS the whole BVP\n"
        "  composite — the estimator skips per-driver scoring and derives every\n"
        "  driver from it. A midpoint 0.5 therefore does not rank the task, it\n"
        "  abstains, and abstentions are indistinguishable from real scores in\n"
        "  `fw bvp --quadrant`. Measured 2026-08-29: 38 of 41 inceptions still\n"
        "  carried the template's 0.5 and shared one identical BVP of 126.\n"
        "  This message used to offer `voi_score: 0.5` as its worked example while\n"
        "  the line above promised the estimator could rank them — the two halves\n"
        "  contradicted each other and 38 authors reasonably followed the example.\n\n"
