---
id: T-495
name: "unwired-guard census counts a tool named in a COMMENT as a live edge, so a
  discussed tool reads as wired"
description: >
  Reachability is textual, and 46 of 115 tool-to-tool reference lines (40 percent)
  open as comments. tools/_t418-producer-attribution.py is reached from the live root
  _t420-rail-attribution-gate.py purely by the docstring sentence naming it as a compensating
  control — prose about a control, not a call — so an instrument that has not run
  since 2026-08-09 reports as wired. This is the FALSE NEGATIVE direction of the census
  LIMIT, which T-493 documented but deliberately did not fix: stripping comments per
  language (py/sh #, mjs // and /* */) is a change of definition that would move the
  count a third time in one day. Found via T-493.
