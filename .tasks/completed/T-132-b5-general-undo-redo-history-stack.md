---
id: T-132
name: "B5 General undo-redo history stack"
description: >
  T-128 GO decomposition B5: generalize the editor Tidy-only undo into a full undo/redo
  history stack
...
cost_estimate_proposed:
  - ts: '2026-08-16T13:57:16Z'
    ...
    rationale: blast_radius=5 
      (paths:build/gallery/designer.html,docs/reports/T-128-editor-persistence-inception.md,src/aef-workflow-designer.html,tools/_undo-verify-cdp.mjs);
...
Design fixed in T-128 IW-4 / Decisions and `docs/reports/T-128-editor-persistence-inception.md`:
per-gesture granularity, snapshot-BEFORE at explicit mutation sites via `pushHistory(label)`,
NOT hooked into `renderAll()` (fires on non-mutating re-renders → spurious entries; autosave stays
in renderAll, being idempotent). Full-state snapshots (maps are ~14 nodes — cheap), stack capped ~50.
...
### Human
- [x] [REVIEW] Undo/redo feels right in normal editing.
  **Steps:** move a few nodes, add/delete one, then press Ctrl+Z several times, then Ctrl+Shift+Z.
  **Expected:** each Ctrl+Z steps back exactly one gesture; redo re-applies. **If not:** note which
  action didn't undo cleanly (e.g. a drag that took multiple undos).
...
### 2026-07-06T12:53:48Z — task-created [task-create-agent]
- **Action:** Created task via task-create agent
- **Output:** /opt/832-Workflow-designer/.tasks/active/T-132-b5-general-undo-redo-history-stack.md
