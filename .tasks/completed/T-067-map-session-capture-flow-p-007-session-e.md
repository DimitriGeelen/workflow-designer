...
description: >
  Dogfood map of the vendored session-capture agent: session-end trigger -> scan conversation
  per category (work/decisions/learnings/questions/gaps) -> capture writes (tasks/updates/practices)
  -> observation-inbox triage -> capture report -> session may close. Ground truth:
  .agentic-framework/agents/session-capture/AGENT.md ONLY (doctrine-only agent, no
  script; referenced capture.sh does not exist). First 100%-doctrine corpus map; completes
  the session-lifecycle trio (session-handover, resume-status). Produces examples/aef-processes/session-capture.workflow.yaml
  + friction report.
...
date_finished: 2026-07-04T08:36:25Z
...
cost_estimate_proposed:
  - ts: '2026-08-16T13:57:15Z'
    ...
    rationale: blast_radius=5 
      (paths:docs/reports/T-067-session-capture-friction.md,examples/aef-processes/session-capture.workflow.yaml,tests/run-bridge-tests.sh,tools/check-lane-bands.py);
...
### 2026-07-04T08:36:25Z — status-update [task-update-agent]
