---
id: T-578
name: "The unwired-guard census strips Python and shell comments but not JavaScript
  block comments"
description: >
  T-495 eliminated prose false-edges for Python (tokenize + ast) and shell (word-aware
  hash), leaving .mjs/.js untouched. Consequence measured under T-423: tools/_t423-carrier-agreement-guard.py
  is counted WIRED solely because a JSDoc comment in its sibling .mjs spells out the
  path; reword that comment and a live guard reports unwired. The census LIMIT paragraph
  enumerates heredocs, md/yaml/json roots and multi-line shell strings, and does not
  name this one - so the blindness is not merely present, it is absent from the list
