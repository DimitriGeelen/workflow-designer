---
id: T-129
name: "B2: Editor Save-to-repo sidecar with versioned snapshot store"
description: >
  B2: Editor Save-to-repo sidecar with versioned snapshot store
...
date_finished: 2026-07-06T12:42:14Z
...
bvp_scores_proposed:
  ...
  - ts: '2026-08-16T14:33:14Z'
    ...
    rationale: D1=4 (body:structural-gate); D2=0 (no-signal); D3=2 
      ...
      F3=1 (prose:AEF seam-incidental); F1=2 (prose:process-editor-capability)
    rubric_sha: e4a00f38e801
cost_estimate_proposed:
  - ts: '2026-08-16T13:57:16Z'
    ...
    rationale: blast_radius=3 
      (paths:tools/_save-api-verify.mjs,tools/gallery-serve.py,tools/serve-gallery.sh);
      tier=2 (no-signal); effort=8 (no-signal)
    rubric_sha: e4a00f38e801
---

# T-129: B2: Editor Save-to-repo sidecar with versioned snapshot store

## Context

T-128 GO decomposition, task B2. The write-capable backend for editor persistence: a localhost
Python sidecar that serves the designer AND `/api/*`, so the editor can save the current document
durably into the repo with per-map versioning. B3 (Save button) and B4 (revert UI) consume it.

**Build-time refinement of T-128 D1 (store format):** the editor emits BPMN XML (`buildBpmnXml`)
and there is no bpmn→yaml converter; `workflow.yaml` is the human-authored *semantic* source
(it carries `x/y`, but writing it back would need lossy reconstruction). So the version store uses
**BPMN** as the snapshot format — native to the editor, no lossy conversion, geometry preserved
exactly, and diffable for the future learning arc. Save writes `rendered/<id>.bpmn` (the gallery's
geometry artifact) — NOT the semantic `workflow.yaml` (left to humans). This is MORE
sovereignty-respecting than D1, not less. Recorded in ## Decisions.
...
node tools/_save-api-verify.mjs
...
### 2026-07-06T12:36:01Z — task-created [task-create-agent]
- **Action:** Created task via task-create agent
- **Output:** /opt/832-Workflow-designer/.tasks/active/T-129-b2-editor-save-to-repo-sidecar-with-vers.md
- **Context:** Initial task creation

### 2026-07-06T12:42:14Z — status-update [task-update-agent]
