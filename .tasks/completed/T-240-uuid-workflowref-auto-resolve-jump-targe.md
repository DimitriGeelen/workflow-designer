...
description: >
  AEF field observation (rail offset 166, their T-2611, non-blocking): a linkEventThrow
  with uuid workflowRef + name shows 'Target workflow — none —' and a disabled jump
  after a ?load deep-link load, until the operator re-binds via Choose-from-project;
  legacy slug refs bind directly. AEF's whole corpus is uuid-form post-recreate, so
  this costs one picker step per jump per session. Candidate fix (AEF-suggested):
  resolve uuid->project via /api/list at load/import time and bind the jump target
  when exactly one live map matches the workflowRef.
...
bvp_scores_proposed:
  ...
  - ts: '2026-08-16T14:33:22Z'
    ...
    rationale: D1=4 (body:structural-gate); D2=0 (no-signal); D3=0 (no-signal); 
      D4=3 (body:portability-abstraction); F-RECALL=2 (body:lightly-promoted); 
      F2=0 (no-signal); F4=1 (prose:routing/geometry-incidental); F3=1 
...
cost_estimate_proposed:
  - ts: '2026-08-16T13:57:19Z'
    ...
    rationale: blast_radius=5 
      (paths:src/aef-workflow-designer.html,tests/test_editor_behavior.py,tools/_editor-behavior-verify-cdp.mjs,tools/gallery-serve.py);
...
### Human
- [x] [REVIEW] The auto-resolved readout reads clearly in the properties panel
      **Steps:**
