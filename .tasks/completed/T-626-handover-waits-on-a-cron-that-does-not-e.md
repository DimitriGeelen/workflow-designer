---
id: T-626
name: "Handover waits on a cron that does not exist here to run a 14ms scan, so every
  dated revisit trigger is silent"
description: >
  Handover waits on a cron that does not exist here to run a 14ms scan, so every dated
  revisit trigger is silent
...
cost_estimate_proposed:
  - ts: '2026-09-21T20:25:08Z'
    ...
    rationale: blast_radius=3 
      (paths:.agentic-framework/agents/context/revisit-due-scan.sh,.agentic-framework/agents/handover/handover.sh);
