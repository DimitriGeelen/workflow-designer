---
id: T-063
name: "Represent structured META_KEYS aef values (emits/aggregation/compensates/multiInstance/timer)"
description: >
  T-062 hardening (WARN on structured value in the scalar aef:meta channel) surfaced
  5 KNOWN META_KEYS keys carrying dict/list values silently dropped since authoring:
  emits (list, x5 in audit-process), aggregation (dict), compensates (list), multiInstance
  (dict), timer (dict). Unlike T-062 one-offs these want a STRUCTURED representation
  (dedicated aef child element like io/decisionInput), not flattening. Decide per
  key: structured emit vs flatten. Then corpus runs fully warn-clean (0 'cannot ride'
  lines too). Depends on T-062.
...
cost_estimate_proposed:
  - ts: '2026-08-16T13:57:15Z'
    ...
    rationale: blast_radius=5 
      (paths:docs/reports/T-062-aef-key-reconciliation.md,tests/run-bridge-tests.sh,tests/test_editor_bridge_structured_parity.py,tools/yaml-to-bpmn.py);
      tier=2 (no-signal); effort=8 (no-signal)
    rubric_sha: e4a00f38e801
---

# T-063: Represent structured META_KEYS aef values (emits/aggregation/compensates/multiInstance/timer)

## Context

The T-062 bridge hardening (WARN when a `META_KEYS`/`aef.x-*` key holds a dict/list that
can't ride the scalar `<aef:meta>` attribute channel) surfaced 5 KNOWN `META_KEYS` keys
that carry structured values in the corpus and have been silently dropped since authoring:
`emits` (list, ×5 in audit-process), `aggregation` (dict), `compensates` (list),
`multiInstance` (dict, revisit-due-scan), `timer` (dict). Unlike the T-062 one-offs these
name real modelling concepts that *want* structure (a list of emitted events, a timer
config, a compensation set) — flattening to a scalar would be lossy. The right fix is a
dedicated structured emit path (like `io`/`decisionInput`), editor-side read+write, and a
seam guard. See docs/reports/T-062-aef-key-reconciliation.md §"Bridge hardening".
...
### 2026-07-04T00:15:00Z — task-created [task-create-agent]
- **Action:** Created task via task-create agent
- **Output:** /opt/832-Workflow-designer/.tasks/active/T-063-represent-structured-metakeys-aef-values.md
