---
id: T-455
name: "Recommendation block: the guard requires a section default.md never supplies"
description: >
  tools/_norec-verify.py blocks handover on a parseable '## Recommendation' block
  carrying **Recommendation:** GO/NO-GO/DEFER/ABSTAIN, but .tasks/templates/default.md
  - the template every build task is stamped from - has no ## Recommendation section
  at all. Only inception.md documents the verdict line (:117). Measured 2026-08-12:
  121 task files carry the heading and 120 spell the verdict parseably, i.e. the convention
  is near-universal and was acquired entirely by imitation. The cost is not hypothetical:
  3 of the 14 NO-REC tasks T-454 repaired had already been ANALYSED and carried a
  decided verdict, invisible to the operator's queue purely because the author invented
  a different shape - T-209 nested it as '### Recommendation' inside ## Context, T-286
  opened '**[GO]**', T-341 opened 'Option **(2)**'. Same class as T-453: a gate measured
  against the shipped template rather than against a fixture. Vendored file, so the
  fix is AEF's under G-008.
...
cost_estimate_proposed:
  - ts: '2026-08-16T13:57:23Z'
    ...
    rationale: blast_radius=5 
      (paths:.tasks/templates/default.md,.tasks/templates/inception.md,tools/_t352-p011-errexit-probe.sh,tools/validate-workflow.py);
  ...
  - ts: '2026-08-16T13:58:56Z'
    ...
    rationale: blast_radius=3 
      (paths:.tasks/templates/default.md,.tasks/templates/inception.md); tier=2 
      (no-signal); effort=8 (no-signal)
    rubric_sha: e4a00f38e801
---

# T-455: Recommendation block: the guard requires a section default.md never supplies
...
# 1. The gap, read off the SHIPPED template rather than a fixture (T-453 discipline).
test 0 -eq "$(grep -c '^## Recommendation' .tasks/templates/default.md)"
# 2. The remedy already exists in a sibling template — so this is "copy the section that
#    works", not a design question.
test 1 -le "$(grep -c '^## Recommendation' .tasks/templates/inception.md)"
test 1 -le "$(grep -c 'Recommendation:\*\* GO / NO-GO / DEFER' .tasks/templates/inception.md)"
# 3. NO local patch to the vendored template. An empty diff is the deliverable here:
#    a local fix is silently reverted by the next bump and reads as fixed meanwhile.
...
### 2026-08-12T12:19:19Z — task-created [task-create-agent]
- **Action:** Created task via task-create agent
- **Output:** /opt/832-Workflow-designer/.tasks/active/T-455-recommendation-block-the-guard-requires-.md
