...
description: >
  A second DM topic went unread for ~24 days while I repeatedly reported 'AEF is silent'
  from a one-topic measurement. termlink_agent_inbox cannot distinguish all-clear
  from an untracked cursor store, so the natural check is uninformative. Need a rail
  sweep that enumerates all topics carrying my fingerprint and refuses to report all-clear
  from an empty cursor store.
...
cost_estimate_proposed:
  - ts: '2026-08-16T13:57:21Z'
    ...
    rationale: blast_radius=5 
      (paths:tools/_t352-p011-errexit-probe.sh,tools/_t360-rail-sweep-teeth.py,tools/rail-sweep.py,tools/validate-workflow.py);
  ...
  - ts: '2026-08-16T13:58:53Z'
    ...
    rationale: blast_radius=3 
      (paths:tools/_t360-rail-sweep-teeth.py,tools/rail-sweep.py); tier=2 
