---
id: T-176
name: "Offline-harden the designer: inline or self-host fonts (remove Google Fonts
  CDN dependency)"
description: >
  Surfaced by T-174: the designer links Google Fonts (fonts.googleapis.com/gstatic).
  Authoring functions offline (system-font fallback) but is not zero-network, which
  matters for air-gapped AEF deployments vendoring the released artifact. Inline the
  woff2 fonts as base64 (or self-host) so the single file is truly self-contained.
  Visual change to src/ — needs visual verification across modes.
...
cost_estimate_proposed:
  - ts: '2026-08-16T13:57:17Z'
    ...
    rationale: blast_radius=5 
      (paths:scripts/embed-fonts.py,src/aef-workflow-designer.html,tests/test_designer_owner_derived.py,tests/test_editor_bridge_meta_parity.py);
...
-->
- [x] [REVIEW] Embedded fonts render identically to the CDN version, no visible regression
  **Steps:**
