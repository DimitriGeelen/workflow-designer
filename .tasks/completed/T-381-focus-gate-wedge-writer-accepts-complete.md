---
id: T-381
name: "focus gate wedge: fw context focus accepts a completed task id its own reader rejects"
description: >
  `fw context focus <id>` validated existence (active/ OR completed/) while the task
  gate requires activeness (active/ only), so focusing a completed task succeeded and
  then blocked every gated Write/Edit/Bash. Writer now requires what its reader requires.
  Filed originally with a second claim — that the write-pattern precheck disarmed the
...
last_update: 2026-08-08T15:25:37Z
