...
description: >
  Paint a theme-surface halo rect (getBBox-sized, small padding) behind edge labels
  and node id badges so text stays legible when a line must pass under it. Cheap,
  catches the residue the placement pass (edge-label task) cannot move. From context-memory
...
### Human
- [x] [REVIEW] Badges and edge labels stay legible where lines must pass under them
  **Steps:**
