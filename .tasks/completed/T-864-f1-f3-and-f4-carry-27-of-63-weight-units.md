...
owner: agent
horizon: null
tags: [bvp, false-green, arc:arc-003]
components:
...
last_update: 2026-09-26T09:08:18Z
date_finished: 2026-09-26T09:08:18Z
...
### Agent
- [x] F1, F3 and F4 each carry a `scoring:` block of `kind: signals`, and each validates: `fw bvp driver --validate-scoring` reports VALID for all three
- [x] Each also carries the `rubric:`, `polarity:` and `guardrails:` blocks that F2 and F-RECALL already have, so a human can read what the machine matches on
...
python3 -c "import yaml; d=yaml.safe_load(open('policy/value-drivers.yaml')); ids=set(e['id'] for e in d['free_drivers'] if 'scoring' in e); assert ids>={'F1','F3','F4'}, ids"
python3 -c "import yaml; d=yaml.safe_load(open('policy/value-drivers.yaml')); bad=[e['id'] for e in d['free_drivers'] if 'scoring' in e and not (e.get('rubric') and e.get('guardrails'))]; assert not bad, bad"
out=$(.agentic-framework/bin/fw bvp driver --explain F4 T-358 2>&1); echo "$out" | grep -q '^Score: 4'
out=$(.agentic-framework/bin/fw bvp driver --explain F4 T-737 2>&1); echo "$out" | grep -q '^Score: 0'
...
### 2026-09-26 — the scorer is not the deliverable; the re-rank is

- **What changed:** `--explain` showed all three drivers matching correctly while `fw bvp`
  showed the ranking completely unmoved. NORM is computed from the STORED `bvp_scores:`
  dict, not recomputed from the spec, so a driver can be fully working and still contribute
  nothing until each task is re-scored. Had I stopped at a green `--explain` I would have
  reported a fix that changed no decision anywhere — the exact false-green this arc is about.
- **Discrimination measured** (the check that matters):
  ...
  | T-696 voi_score | 2 | 0 | 1 | remediation |
...
### 2026-09-26T07:53:42Z — task-created [task-create-agent]
...
- **Scan ID:** R-bbb0d4a3
- **Timestamp:** 2026-09-26T09:08:20Z
- **Catalogue:** v1.3-seed
- **Overall:** PASS
- **Needs Human:** no
- **Findings:** none

### 2026-09-26T09:08:18Z — status-update [task-update-agent]
