---
id: T-843
name: "Gate uncontrolled absence assertions at the close path: move the detector to the point of admission"
description: >
...
# T-843: Gate uncontrolled absence assertions at the close path: move the detector to the point of admission
...
- [x] **UNPARSEABLE LEGS REPORT `NOT EVALUATED`, NEVER PASS AND NEVER FAIL.** The census
      ...
      ("The check ran and found nothing… A PASS here would assert coverage the check does not
...
# ── T-843 legs ────────────────────────────────────────────────────────────────
# The pre-written test suite passes, and leg 2 asserts the suite could MEASURE — without it
# the "expect empty output" tests pass vacuously whenever the classifier is missing, which
# is exactly how the first run of that file reported three false greens.
out=$(bash tools/_t843-absence-gate-tests.sh 2>&1); echo "$out" | grep -q '^# passed ' && ! echo "$out" | grep -q '^not ok'
out=$(bash tools/_t843-absence-gate-tests.sh 2>&1); echo "$out" | grep -q 'PRE: the classifier resolved'

# The GATE bites, not just the predicate under it — and the remedy its message PRINTS is
# executed and shown to clear it. Writing a third check whose named remedy cannot be applied
# (after OBS-374 and the bvp verb gap) would have been indefensible.
...
grep -q '_fw_absence_classifier' .agentic-framework/lib/verification-absence.sh
grep -q -- '--block --uncontrolled' .agentic-framework/lib/verification-absence.sh

# The gate is wired into the close path beside its two precedents.
grep -q 'check_verification_uncontrolled_absence "\$verify_cmds"' .agentic-framework/agents/task-create/update-task.sh

# The block message names its own escape (a gate that refuses without naming its escape is
# PL-304) and both repair routes.
out=$(sed -n '/^check_verification_uncontrolled_absence/,/^}/p' .agentic-framework/agents/task-create/update-task.sh); echo "$out" | grep -q 'FW_ALLOW_UNCONTROLLED_ABSENCE=1 (logged Tier-2)'
out=$(sed -n '/^check_verification_uncontrolled_absence/,/^}/p' .agentic-framework/agents/task-create/update-task.sh); echo "$out" | grep -q 'NOT EVALUATED'
...
# something; and test -f covers the weaker PATH case as a separate control.
grep -q 'FW_ALLOW_UNCONTROLLED_ABSENCE' .agentic-framework/agents/task-create/update-task.sh
...
### 2026-09-25T09:27:29Z — task-created [task-create-agent]
