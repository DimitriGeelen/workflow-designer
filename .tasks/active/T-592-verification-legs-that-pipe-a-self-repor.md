---
id: T-592
name: "Verification legs that pipe a self-reporting command into grep discard its
  exit status"
description: >
  Verification legs that pipe a self-reporting command into grep discard its exit
  status
...
cost_estimate_proposed:
  - ts: '2026-09-21T20:24:52Z'
    ...
    cost_estimate:
      ...
      effort: 8
      blast_radius: 1
    rationale: blast_radius=1 (paths:tools/check-vacuous-verification.py); 
...
<!-- Criteria requiring human verification (UI/UX, subjective quality). Not blocking.
     ...
     [REVIEWER] example (static-scan-verifiable — convert to Agent AC + Verification):
       - [ ] [REVIEWER] Block message names both bypass mechanisms
         **Steps:**
         1. Run `bin/fw reviewer T-592`
         **Expected:** Verdict: PASS; no findings on `block-message-completeness`
         **If not:** Inspect hook block-message string and add missing mechanism
       Conversion: this AC should be moved to ### Agent and
       `bin/fw reviewer T-592 2>&1 | grep -q "Overall:.*PASS"` added to ## Verification.
-->
