---
id: T-867
name: "S2: the agent drafts the hypothesis, the human corrects it, and the correction
  is sticky"
description: >
  The operator said formulating a hypothesis is difficult; a blank required field
  would therefore be a gate that blocks rather than helps. So the estimator drafts
  one from the task's own text, the human edits it, and hypothesis_source: human makes
  the edit permanent against every later automatic pass. Reuses the sticky-provenance
  mechanism shipped in T-865 for voi_score and target_blast_radius — same two-state
...
# T-867: S2: the agent drafts the hypothesis, the human corrects it, and the correction is sticky
...
### 2026-09-26T12:22:55Z — task-created [task-create-agent]
