...
Usage:
    ...
    python3 tools/_t703-inbox-residue.py --urgent --check --write PATH   # T-702: all urgent, any age
"""
...
def residue(obs, now, min_age=STALE_DAYS, urgent_only=False):
    idx = task_index()
    rows = []
    for o in obs:
        if o.get("status") != "pending":
            continue
        if urgent_only and o.get("urgent") is not True:
            continue
        cap = o["captured"]
...
def main():
    ...
    ap.add_argument("--urgent", action="store_true",
                    help="T-702 (RA-006): every pending URGENT item, any age, with its reason class")
    ...
    obs = load()
    rows = residue(obs, now, min_age=-1, urgent_only=True) if a.urgent else residue(obs, now)
    pending = sum(1 for o in obs if o.get("status") == "pending")
    label = "urgent pending (any age)" if a.urgent else f"pending > {STALE_DAYS}d"
    census = {}
    for r in rows:
    ...
    if a.write:
        with open(a.write, "w") as fh:
            fh.write(f"# {'T-702 — urgent observations, individually named' if a.urgent else 'T-703 — observation-inbox residue, enumerated'}\n\n")
            fh.write(f"Generated by `tools/_t703-inbox-residue.py --write` on {now.date()} — "
                     f"regenerate, do not hand-edit.\n\n")
            fh.write(f"Pending: {pending} of {len(obs)}. Rows ({label}): {len(rows)}.\n\n")
    ...
    print(f"T-703 — inbox residue: {len(rows)} {label} (of {pending} pending)")
    for k, v in sorted(census.items(), key=lambda kv: -kv[1]):
        print(f"  {v:4d}  {k}")
    ...
    if a.check:
        if missing:
            ...
            return 1
        print(f"OK: every one of the {len(rows)} items carries a reason class")
    return 0
