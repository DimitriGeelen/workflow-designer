...
def main():
    ...
    print('  FALSE NEGATIVE — T-495 closed the two prose forms in Python and shell (comments,')
    print('    and string literals that are whole statements). Still open, same direction,')
    print('    WITH THE SIZE OF EACH, because a limitation stated without its scale reads as')
    print('    an edge case and this one is not (T-578 measured the tree):')
    print('      * .md/.yaml/.json roots are read whole — no comment syntax assumed.')
    print('        THIS IS THE BIG ONE: 110 of the 237 referenced tools (46%) have NO')
    print('        executable-code edge anywhere. Their WIRED verdict rests entirely on')
    print('        prose — task files 214 refs, handovers 206, episodic 130. For those')
    print('        tools "wired" is a fact about what a handover once said, not what runs.')
    print('      * .mjs/.js are read whole too — JavaScript comments were never stripped,')
    print('        so `// tools/x` and a JSDoc block both read as calls. Small: 5 refs, and')
    print('        0 tools are held by a JS comment ALONE (watched by _t578\'s suite leg).')
    print('      * a shell HEREDOC body reads as an edge (needs a shell parser) — unmeasured')
    print('      * a shell string spanning lines re-opens quote state per line — unmeasured')
    print('  Stated so a clean run cannot imply coverage it does not have (PL-148) — and')
    print('  QUANTIFIED because the previous wording was true sentence-by-sentence and')
    print('  misleading in aggregate, which is a subtler way for a disclosure to fail.')
    if PARSE_FALLBACKS:
